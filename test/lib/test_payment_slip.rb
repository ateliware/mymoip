...
class TestPaymentSlip < Test::Unit::TestCase
  def test_validate_length_of_instruction_lines
    subject = MyMoip::Fixture.payment_slip()
  ...
  def test_validate_expiration_days
    subject = MyMoip::Fixture.payment_slip(expiration_days: 99)
    assert subject.valid?
  ...
  def test_validate_expiration_type
    subject = MyMoip::Fixture.payment_slip(expiration_days_type: :calendar_day)
    assert subject.valid?
  ...
  def test_validate_logo_url
    subject = MyMoip::Fixture.payment_slip(logo_url: 'http://www.uol.com.br')
    assert subject.valid?
  ...
  def test_validate_expiration_date_format
    subject = MyMoip::Fixture.payment_slip(expiration_date: DateTime.new)
    assert subject.valid?
